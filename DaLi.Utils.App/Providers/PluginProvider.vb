...
Namespace Provider
	...
	Public Class PluginProvider
		...
		Public Sub New()
			...
			' 从启动程序集获取所有插件基类的接口
			SystemAssemblies = ReflectionHeler.CurrentAssemblies.ToImmutableList
		...
		Public Shared Function IsPlugin(type As Type) As Boolean
			Return type?.IsComeFrom(GetType(IPlugin))
		End Function

		''' <summary>检查类型是否来自 IPlugin 接口</summary>
		Public Shared Function IsPlugin(Of T)() As Boolean
		...
		Public Sub UpdateLoad(ParamArray newTypes As Type()) Implements IPluginsProvider.UpdateLoad
			...
			SyncLock _Lock
				...
				For Each Ass In SystemAssemblies
					...
					ReflectionHeler.GetTypes(Ass, True).
					...
					types?.ForEach(Sub(type)
									   Dim list As ImmutableList(Of Type) = Nothing
									   If Not Dic.TryGetValue(Ass, list) Then
										   list = ImmutableList.Create(Of Type)
										   Dic.Add(Ass, list)
									   End If
		...
		Public Function GetInterface(ass As Assembly, baseType As Type) As List(Of Type)
			Dim data As IEnumerable(Of Type) = Nothing
			If Instance.TryGetValue(ass, data) Then
				Return data.Where(Function(x) x.IsComeFrom(baseType)).ToList
			Else
				Return Nothing
			End If
		End Function
		...
		Public Sub Update(ParamArray assemblyNames() As String) Implements IPluginsProvider.Update
			If assemblyNames.IsEmpty Then Return
		...
		''' <summary>禁用类型，支持 like 通配符匹配</summary>
		Public Sub Disabled(ParamArray typeNames() As String) Implements IPluginsProvider.Disabled
			If typeNames.IsEmpty Then Return

			SyncLock _Lock
				Instance = Instance.ToDictionary(Function(x) x.Key,
												 Function(x) x.Value.Where(Function(t) Not t.FullName.Like(typeNames)).ToImmutableList).
									ToImmutableDictionary
			End SyncLock
		End Sub

		''' <summary>调试信息，输出所有加载的类型</summary>
		Public Function DebugInfo() As NameValueDictionary Implements IPluginsProvider.DebugInfo
			Return Instance.ToNameValueDictionary(Function(x) x.Key.Name, Function(x) x.Value.Select(Function(y) y.FullName).JoinString(", "))
		End Function
	End Class
