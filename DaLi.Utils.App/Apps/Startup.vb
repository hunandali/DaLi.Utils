...
Namespace App
	...
	Friend Class Startup
		...
		Public Sub Run(sys As SystemApp, Optional args() As String = Nothing)
			' 启动信息
			CON.AppStart($"{sys.Name} / {sys.ID} 启动！({DATE_NOW_STR})")
			...
			' 移除禁用的插件
			Dim pluginSetting = sys.GetSetting(Of PluginSetting)
			sys.Plugins.Remove(pluginSetting.RemoveAssembly)
			sys.Plugins.Disabled(pluginSetting.DisabledTypes)

			' 隐藏部分参数模块
			sys.SettingProvider.Hidden(pluginSetting.HideSettings)

			' 初始化日志对象
			Log.Logger = sys.GetSetting(Of LogSetting).CreateLogger

			' 启动
			Log.Logger.Information("正在初始化运行环境，系统即将启动! {DATE_NOW_STR}", DATE_NOW_STR)
			...
			Try
				...
				' 记录项目完成
				Log.Logger.Information("★ 系统已经结束，累计运行：{long} ★", DATE_FULL_NOW.ShowDiff(SYS_START))
			...
			' 结束提示
			CON.AppFinish($"{sys.Name} ，结束！({DATE_NOW_STR})", RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
		End Sub
...
#Region "相关操作"
		...
		Private Shared Sub ShowSuccessInfo()
			...
			CON.Err($"{vbTab}启动时间： {DATE_NOW_STR("yyyy年MM月dd日HH时mm分")}")
			...
			If SYS.Debug Then
				Log.Debug("当前插件加载明细")
				SYS.Plugins.DebugInfo.
					OrderByDescending(Function(x) x.Key).
					ToList.
					ForEach(Sub(kv)
								Log.Debug("{assembly}：{types}", kv.Key, kv.Value)
							End Sub)
				CON.Echo()
				CON.Line()
			End If

			Log.Information("★ 系统已经启动 ★")
		End Sub
...
#Region "注册系统"
		...
		Private Shared Sub RegisterSystemInfo()
			Call New DeviceStatusProvider().Register(SYS.ID, SYS.Name, "Server", Environment.MachineName, NetHelper.ServerIP, SYS.Information)
			Log.Information("系统信息已经注册到 Redis")
		End Sub

		''' <summary>注销系统信息</summary>
		Private Shared Sub UnregisterSystemInfo()
			Call New DeviceStatusProvider().Unregister(SYS.ID)
			...
			services.AddControllers().
				AddJsonOptions(Sub(opt)
								   With opt.JsonSerializerOptions
									   ...
									   .Converters.Add(New JsonBooleanConverter)
...
#Region "注入 多语言"
			...
			Dim stringLocalizer = resourceManager.Create("Resource", GetType(App.Startup).Assembly.Name)
...
#Region "手工注入 MVC"
			...
			If Not SYS.ConfigureServices(services) Then
				Dim iMvc = services.AddMvc(Sub(options)
											   ...
											   ' 使用路由器前缀
											   If CommonSetting.RoutePrefix.NotEmpty Then options.Conventions.Insert(0, New RoutePrefix(CommonSetting.RoutePrefix))
										   End Sub)
...
#Region "注入后台服务"
			...
			services.AddHostedService(Of BackService)
#End Region

#Region "自动注入接口"

			' Singleton 类服务
			' 每次都是获得同一个实例，单一实例模式：单一实例对象对每个对象和每个请求都是相同的，可以说是不同客户端不同请求都是相同的
			' 项目启动-项目关闭 相当于静态类 只会有一个
			For Each type In SYS.Plugins.GetTypes(Of IServiceSingleton)
				If Not type.IsGenericType Then services.AddSingleton(type)
			Next

			' Scoped 类服务
			' 对于同一个请求返回同一个实例，不同的请求返回不同的实例，作用域模式：作用域对象在一个客户端请求中是相同的，但在多个客户端请求中是不同的
			' 请求开始-请求结束 在这次请求中获取的对象都是同一个
			For Each type In SYS.Plugins.GetTypes(Of IServiceScoped)
				If Not type.IsGenericType Then services.AddScoped(type)
			Next

			' Transient 类服务
			' 每次 service 请求都是获得不同的实例，暂时性模式：暂时性对象始终不同，无论是不是同一个请求（同一个请求里的不同服务）同一个客户端， 每次都是创建新的实例
			' 请求获取-（GC回收 - 主动释放） 每一次获取的对象都不是同一个
			For Each type In SYS.Plugins.GetTypes(Of IServiceTransient)
				If Not type.IsGenericType Then services.AddTransient(type)
			Next
				...
				If CommonSetting.StaticFolder.NotEmpty Then
					For Each Folder In CommonSetting.StaticFolder
						If Folder.Key.NotEmpty AndAlso Folder.Value.NotEmpty Then
							Dim path = Folder.Key
							If Not path.StartsWith("/"c) Then path = "/" & path
