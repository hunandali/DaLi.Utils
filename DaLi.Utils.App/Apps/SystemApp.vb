...
Namespace App
	...
	##     ## ##    ## ########  ######## ###    ###    #### ##        
		...
		Public ReadOnly Property Runtimes As TimeSpan
			Get
				Return DATE_FULL_NOW.Subtract(Start)
			End Get
		End Property
...
#Region "全局服务提供器"
		...
		Public Sub SetServices(services As IServiceCollection)
			_ServiceTypes = services.
				Where(Function(x) Not ReflectionHeler.IsInvalidName(x.ServiceType.FullName, False)).
...
#Region "系统信息及标识"
		...
		Private Function GetID() As Long
			...
			If nvs("version") <> Version Then
				nvs("version") = Version
				nvs("versions") &= $"[{DATE_NOW_STR}] {nvs("version")} ;"
			End If

			' 保存路径发生变化，记录历史路径
			If nvs("root") <> PathHelper.Root Then
				nvs("root") = PathHelper.Root
				nvs("roots") &= $"[{DATE_NOW_STR}] {nvs("root")} ;"
			End If
