...
Namespace Extension
	...
	Public Module HttpContextExtension
		...
		Private Const CONTENT_DATA_NVS = "_Context_Data_Nvs_"
...
#Region "获取请求的值"
		...
		''' <summary>获取所有请求数据原始文本数据</summary>
		<Extension>
		Public Function RequestNameValues(this As HttpContext) As NameValueDictionary
			Return If(this?.Request.RequestNameValues, New NameValueDictionary)
		End Function

		''' <summary>获取所有请求数据</summary>
		<Extension>
		Public Function RequestNameValues(this As HttpRequest) As NameValueDictionary
			If this IsNot Nothing Then
				' 从请求缓存数据中获取
				Dim result = this.HttpContext.ContextItem(Of NameValueDictionary)(CONTENT_DATA_NVS)
				If result IsNot Nothing Then Return result

				' 不存在，分析
				result = New NameValueDictionary
				...
				If Forms.NotEmpty Then
					...
					Next
				Else
					If this.HasJsonContentType Then
						Try
							Dim json = this.Content.ToJsonNameValues
							If json.NotEmpty Then result.AddRange(json)
				...
				this.HttpContext.ContextItem(CONTENT_DATA_NVS, result)
				Return result
			End If
		...
		Public Function ContextItem(Of T)(this As HttpContext, name As Object) As T
			If this Is Nothing OrElse name Is Nothing Then Return Nothing

			Dim value = Nothing
			this.Items.TryGetValue(name, value)

			Return ChangeType(Of T)(value)
		End Function
		...
		Public Function FormEx(this As HttpRequest, key As String) As StringValues
			If key.NotEmpty Then
				Dim value As StringValues = Nothing
				If this.FormEx?.TryGetValue(key, value) Then Return value
			End If
...
#Region "从请求的值中获取数据"
		...
		Public Function Token(this As HttpRequest, ParamArray tokenFieldName() As String) As String
			' 查询字段
			Dim names = {"Authorization", "Token"}
			If tokenFieldName.IsEmpty Then
				tokenFieldName = names
			Else
				tokenFieldName = tokenFieldName.Union(names).Distinct(Function(x) x.ToLowerInvariant).ToArray
			End If
