...
Namespace Model
	...
	Public Class ErrorMessage
		...
		''' <summary>通知消息字段</summary>
		Private Const NOTIFICATIONKEY As String = "_Notification_"
		...
		Public Function Add(name As String, message As String) As ErrorMessage
			If Loc IsNot Nothing Then message = Loc.TranslateWithPrefix(message, "Error.")
			Messages.Add(name, message)
			Return Me
		End Function

		''' <summary>移除错误</summary>
		Public Function Remove(name As String) As ErrorMessage
			If name.NotEmpty AndAlso name <> NOTIFICATIONKEY Then Messages.Remove(name)
			Return Me
		End Function

		''' <summary>移除错误</summary>
		Public Function Remove(ParamArray names() As String) As ErrorMessage
			If names.NotEmpty Then Messages.Remove(names.Where(Function(x) x <> NOTIFICATIONKEY).ToArray)
			Return Me
		End Function
		...
		''' <summary>所有错误字段</summary>
		Public ReadOnly Property Keys As String()
			Get
				Return Messages.Keys.Where(Function(key) key <> NOTIFICATIONKEY).ToArray
			End Get
		End Property

		''' <summary>是否存在指定键的错误</summary>
		Public ReadOnly Property HasError(name As String) As Boolean
			Get
		...
		''' <param name="fullError">当出现异常是，是否包含完整异常信息对象，否则仅返回异常消息说明</param>
		Public Function GetError(Optional fullError As Boolean = False) As Object
			If IsPass Then
				Return Nothing
			ElseIf HasException Then
				If fullError Then Return Exception
