...
Namespace Provider
	...
	Public Class RndCaptchaProvider
		...
		Public Function ValidateCaptcha(code As String) As Boolean Implements ICaptcha.ValidateCaptcha
			...
			If Not Cache.TryGetValue(code, last) Then Return False
...
#Region "验证码"
		...
		''' <summary>生成验证码图片</summary>
		Public Shared Function MakeCodeImage(code As String) As MemoryStream
			If code.IsEmpty Then Return Nothing
				...
				Using canvas = New SKCanvas(bitmap)
					...
					For I = 0 To codeLen - 1
						...
						Using style = New SKPaint
							...
							Using font As New SKFont
								font.Size = fontSize
								font.Typeface = SKTypeface.FromFamilyName(SKTypeface.Default.FamilyName, SKFontStyle.Bold)
								font.MeasureText(s, size) ' 计算文字宽度以及高度

								canvas.DrawText(s, left, top, font, style)
							End Using
						End Using
