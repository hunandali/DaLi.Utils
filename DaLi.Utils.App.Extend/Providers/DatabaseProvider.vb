...
Namespace Provider
	...
	Public Class DatabaseProvider
		...
		Public Sub New(Optional builderAction As Action(Of FreeSqlBuilder) = Nothing)
			...
			' 创建默认数据库连接
			[Default] = MakeConn(VAR_DATABASE_CONNECTION_DEFAULT)
			If [Default] Is Nothing Then Throw New Exception("未设置有效的默认数据库连接参数，或者默认数据库异常，无法连接")

			' 创建日志数据库连接
			Dim logConn = _SettingDatabase.Connections(VAR_DATABASE_CONNECTION_LOG).EmptyValue(VAR_DATABASE_CONNECTION_DEFAULT)
			Log = MakeConn(logConn, builderAction)
			If Log Is Nothing Then Throw New Exception("未设置有效的日志数据库连接参数，或者日志数据库异常，无法连接")

			' 数据连接字典
			Dim Dic As New Dictionary(Of String, IFreeSql) From {{VAR_DATABASE_CONNECTION_DEFAULT, [Default]}, {VAR_DATABASE_CONNECTION_LOG, Log}}
					...
					If connection.NotEmpty Then
						...
						AddHandler Provider.Aop.AuditValue, Sub(s As Object, e As AuditValueEventArgs)
																If (e.Column.CsType Is GetType(Long) OrElse e.Column.CsType Is GetType(ULong)) AndAlso e.Value?.ToString = "0" Then
																	...
																	If pro IsNot Nothing Then
																		...
																		' 其中模型标识 1xxx-39xx 中 39 个模型生成雪花数据时使用 moduleId 字段
																		If Not moduleId.HasValue AndAlso e.Object IsNot Nothing Then moduleId = ExtendHelper.GetModuleId(e.Object.GetType)

																		If moduleId.HasValue AndAlso moduleId.Value < AppModuleProvider.MAX_ID Then
																			moduleId = Math.Floor(moduleId.Value / 100) Mod 32
																			moduleId += 1
																		Else
																			moduleId = 0
																		End If
						...
						AddHandler Provider.Aop.ConfigEntityProperty, Sub(s As Object, e As ConfigEntityPropertyEventArgs)
																		  ...
																		  If e.Property.PropertyType = GetType(NameValueDictionary) OrElse e.Property.PropertyType = GetType(KeyValueDictionary) Then
																			  ...
																			  If e.ModifyResult.StringLength < 16 Then e.ModifyResult.StringLength = -2
																		  End If
																	  End Sub
					End If
				End If
