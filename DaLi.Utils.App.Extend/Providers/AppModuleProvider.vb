...
Namespace Provider
	...
	Public Class AppModuleProvider
		...
		''' <summary>允许系统默认最大手动标识，未手动标记的标识都将大于此标识，并默认非系统模块，禁用</summary>
		Public Const MAX_ID As UInteger = 1000000
		...
		''' <summary>内置模块条件</summary>
		Private ReadOnly _InherModule As String() = {"dali.utils.*", "dali.framework.*"}
		...
		Public Overloads Sub Reload(mustReload As Boolean)
			...
			SyncLock _Lock
				' 获取初始数据
				Dim list = Db.Select(Of ModuleEntity).ToList

				' 是否需要初始化条件
				' 1. 目前数据库没有任何数据
				' 2. 调试模式，每次系统启动都重新初始化
				If list.IsEmpty OrElse (SYS.Debug AndAlso Not _IsInit) Then
					If list.IsEmpty Then list = New List(Of ModuleEntity)

					' 基础数据不存在，需要按系统默认入库一次参数
					' 1xxx-39xx 号模型使用带 moduleId 标识 雪花算法 ID，所以需要此方法的一定要存入低位ID
					' 否则直接使用全 ID 算法
					' 获取所有内置模块
					...
					' 检查未入库的项目
					Dim Names = list.Select(Function(x) x.Name).ToList
					entities = entities.
						Where(Function(x) Not Names.Contains(x.Name)).
						OrderBy(Function(x) x.ID).
						ToList

					' 获取最大标识
					Dim max = Db.Select(Of ModuleEntity).OrderByDescending(Function(x) x.ID).ToOne(Function(x) x.ID)
					If max < MAX_ID Then max = MAX_ID
					...
					' 重新加载
					list = Db.Select(Of ModuleEntity).ToList
				End If

				_ModuleList = list.Where(Function(x) x.Enabled OrElse IsBaseId(x.ID)).ToImmutableDictionary(Function(x) x.ID, Function(x) (x.Name, x.Title, x.Search.SplitEx, x.Audit.SplitEx))
				_IsInit = True
			End SyncLock
		End Sub
		...
		Private Shared Function GetFields(type As Type) As ModuleEntity
			...
			Dim entity As New ModuleEntity With {
				.ID = attr.ID,
				.Name = UpdateName(type),
				.Base = attr.ID < MAX_ID,
				.Enabled = attr.ID < MAX_ID OrElse attr.Enabled,
				.Update = DATE_NOW
			}

			entity.Title = attr.Name.EmptyValue(entity.Name)
			entity.Search = If(attr.Searchable, type.
				GetAllProperties.
				Select(Function(pro)
		...
		Public Function GetModuleName(moduleId As UInteger) As String Implements IAppModuleProvider.GetModuleName
			...
			Dim value As (Name As String, Title As String, Search As String(), Audit As String()) = Nothing
			Return If(ModuleList.TryGetValue(moduleId, value), value.Name, Nothing)
		End Function
		...
		Public Function GetModuleSearch(moduleId As UInteger) As String() Implements IAppModuleProvider.GetModuleSearch
			...
			Dim value As (Name As String, Title As String, Search As String(), Audit As String()) = Nothing
			Return If(ModuleList.TryGetValue(moduleId, value), value.Search, Nothing)
		End Function
		...
		Public Function GetModuleAudit(moduleId As UInteger) As String() Implements IAppModuleProvider.GetModuleAudit
			...
			Dim value As (Name As String, Title As String, Search As String(), Audit As String()) = Nothing
			Return If(ModuleList.TryGetValue(moduleId, value), value.Audit, Nothing)
		End Function
		...
		Public Function ModuleNames(Optional incSystem As Boolean = False) As List(Of String) Implements IAppModuleProvider.ModuleNames
			...
			If incSystem Then
				Return names
			Else
				Return names.Where(Function(x) Not x.Like(_InherModule)).ToList
			End If
		End Function
		...
		Public Function ModuleDatas(Optional mode As TristateEnum = TristateEnum.DEFAULT) As List(Of DataList) Implements IAppModuleProvider.ModuleDatas
			...
			Select Case mode
				...
				Case TristateEnum.FALSE
					list = list.Where(Function(x) Not x.Name.Like(_InherModule))
		...
		Public Function GetModuleInfo(moduleId As UInteger?, moduleValue As Long?) As String Implements IAppModuleProvider.GetModuleInfo
			...
			Dim title = Nothing
			If moduleInfo.TryGetValue("title", title) Then Return title

			Dim name = Nothing
			If moduleInfo.TryGetValue("name", name) Then Return name
		...
		''' <summary>是否基础标识，基础标识不能关闭</summary>
		Public Function IsBaseId(moduleId As Long) As Boolean Implements IAppModuleProvider.IsBaseId
			Return moduleId < MAX_ID AndAlso moduleId >= 0
		End Function
	End Class
