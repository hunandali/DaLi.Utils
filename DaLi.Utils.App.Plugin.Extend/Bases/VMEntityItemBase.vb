...
#Region "编辑对象"
		...
		Public Overridable Function Edit(updateAll As Boolean, skipAudit As Boolean) As T
			Dim entity = ValidateEdit(Me.Entity, updateAll, skipAudit, False)
			If Not ErrorMessage.IsPass Then Return Nothing
		...
		Public Overridable Async Function EditAsync(updateAll As Boolean, skipAudit As Boolean) As Task(Of T)
			...
			Try
				...
				Using repo = Db.GetRepository(Of T)
					...
					Dim succ = Await repo.UpdateAsync(entity) > 0
					If Not succ Then Throw New Exception("更新数据异常")
				End Using
			...
			If flag.Value Then
				...
				Using repo = Db.GetRepository(Of T)
					If EntityCascade Then
						' 基于数据库级联删除，基于对象可能不支持
						succ = repo.DeleteCascadeByDatabase(Function(x) x.ID = Entity.ID) IsNot Nothing
					Else
						' 直接删除
						succ = repo.Delete(Entity) > 0
					End If
				End Using
		...
		Public Overridable Async Function DeleteAsync() As Task(Of Boolean)
			...
			If flag.Value Then
				...
				Using repo = Db.GetRepository(Of T)
					If EntityCascade Then
						' 基于数据库级联删除，基于对象可能不支持
						succ = Await repo.DeleteCascadeByDatabaseAsync(Function(x) x.ID = Entity.ID) IsNot Nothing
					Else
						' 直接删除
						succ = Await repo.DeleteAsync(Entity) > 0
					End If
				End Using
