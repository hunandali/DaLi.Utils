...
Namespace Base
	...
	Public MustInherit Class EntityPluginBase
		Implements IEntityPlugin
...
#Region "字段属性"
		...
		Private Shared _FieldsCache As ImmutableDictionary(Of String, ImmutableDictionary(Of PropertyInfo, ImmutableList(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)))) = ImmutableDictionary.Create(Of String, ImmutableDictionary(Of PropertyInfo, ImmutableList(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String))))

		''' <summary>获取属性列表</summary>
		''' <remarks>返回结果中的 Action 为小写；如果仅空值才行修改，则将过滤掉非空字段</remarks>
		Protected Function GetProperties(Of T As IEntity)(entity As T) As ImmutableDictionary(Of PropertyInfo, ImmutableList(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)))
		...
		Protected ReadOnly Property Properties(type As Type) As ImmutableDictionary(Of PropertyInfo, ImmutableList(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)))
			Get
				Dim cacheName = $"{type.FullName}@@{ProviderName}"
				Dim cacheValue = Nothing
				If _FieldsCache.TryGetValue(cacheName, cacheValue) Then Return cacheValue
				...
				Dim attrs = type.GetEntityCustomAttributes(ProviderName, Not SourceForce)?.
							 ...
							 ToImmutableDictionary(Function(x) x.Key, Function(x) x.Select(Function(y) (y.Target, y.Action?.ToLowerInvariant, y.EmptyOnly, y.data)).ToImmutableList)
		...
		Protected Shared Sub UpdateValue(Of T)(entity As T,
										properties As IEnumerable(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)),
		...
		''' <param name="value">当前值</param>
		Protected Shared Sub UpdateValue(Of T)(entity As T, pro As PropertyInfo, context As IAppContext, value As Object)
			' 设置值
		...
		''' <param name="properties">属性列表</param>
		''' <param name="action">指定的操作</param>
		Protected Shared Function ExistAction(properties As IEnumerable(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)), action As String) As Boolean
		...
		''' <param name="properties">属性列表</param>
		''' <param name="actions">指定的操作</param>
		Protected Shared Function ExistAction(properties As IEnumerable(Of (Target As PropertyInfo, Action As String, EmptyOnly As Boolean, Data As String)), actions As String()) As Boolean
...
#Region "操作"
		...
		Public Overridable Sub ExecuteValidate(Of T As IEntity)(action As EntityActionEnum, entity As T, context As IAppContext, errorMessage As ErrorMessage, db As IFreeSql, Optional source As T = Nothing) Implements IEntityPlugin.ExecuteValidate
		End Sub
		...
		Public Overridable Sub ExecuteFinish(Of T As IEntity)(action As EntityActionEnum, data As ObjectArray(Of T), context As IAppContext, errorMessage As ErrorMessage, db As IFreeSql, Optional source As T = Nothing) Implements IEntityPlugin.ExecuteFinish
		End Sub
		...
		Public Overridable Sub ExecuteQuery(Of T As {Class, IEntity})(action As EntityActionEnum, query As ISelect(Of T), Optional queryVM As QueryBase(Of T) = Nothing) Implements IEntityPlugin.ExecuteQuery
		End Sub
