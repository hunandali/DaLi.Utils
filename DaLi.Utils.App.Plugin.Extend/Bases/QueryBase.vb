...
Namespace Base
	...
	Public Class QueryBase(Of T As {IEntity, Class})
		...
		''' <summary>默认上级字段名称</summary>
		Private ReadOnly _ParentFields As String() = {"ParentIds", "ParentId", "Parent"}
		...
		Public Sub SetSort(fieldName As String, isAsc As Boolean)
			If fieldName.IsEmpty Then Return
		...
		''' <param name="withSort">附加排序</param>
		Public Overridable Sub QueryExecute(query As ISelect(Of T), Optional withSort As Boolean = True)
			' 查询前事件，优先操作
								...
								ElseIf field.fieldType.IsNullableDate Then
									...
									Select Case field.fieldOperator
										Case DynamicFilterOperator.DateRange, DynamicFilterOperator.Range
											...
											If value.NotEmpty AndAlso value.Count > 1 Then
												Dim dateStart = If(value(0).IsValidate, value(0), Date.MinValue)
												Dim dateEnd = If(value(1).IsValidate, value(1), Date.MinValue)
			...
			If EntityType.IsComeFrom(GetType(IEntityParent)) Then
				...
				' 检查是否存在上级参数
				Dim parentName = MyBase.Where(Function(x) _ParentFields.Contains(x.Key, StringComparer.OrdinalIgnoreCase)).Select(Function(x) x.Key).FirstOrDefault
				...
				If sortValue IsNot Nothing Then
					If sortValue.GetType.IsString Then
						...
						sort = sortValue.ToString.
								Split(","c).
								Select(Function(x) x.TrimFull.ToLowerInvariant).
								Distinct.
								Where(Function(x) x.NotEmpty).
								Select(Function(x) $"{x} asc".Split(" "c)).
								Distinct(Function(x) x(0)).
								ToDictionary(Of String, Object)(Function(x) x(0), Function(x) x(1).NotEmpty AndAlso x(1).StartsWith("d"c))
					Else
