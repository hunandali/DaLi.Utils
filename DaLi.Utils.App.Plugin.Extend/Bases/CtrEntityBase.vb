...
#Region "其他相关操作"
		...
		''' <summary>实体字段解码</summary>
		''' <remarks>为了防止防火墙对数据的拦截，对部分文本字段进行了 base64 编码，实际使用前需要进行解码</remarks>
		Protected Function DecodeEntity(entity As T) As T
			If entity Is Nothing Then Return entity

			' 从提交的参数中获取 _keys 字段数据
			Dim keys = AppContext.Fields.GetValue("_encode")?.SplitDistinct
			If keys.IsEmpty Then Return entity

			' 分析字段
			entity.GetType.GetAllProperties.
				Where(Function(x) keys.Contains(x.Name, StringComparer.OrdinalIgnoreCase) AndAlso x.PropertyType.IsString).
				ToList.
				ForEach(Sub(pro)
...
#Region "获取项目"
		...
		<HttpGet("{id}")>
		<ResponseCache(Duration:=5, VaryByQueryKeys:={"id"})>
...
#Region "查询项目"
		...
		Protected Overridable Function QueryDataTree(Of P As {IEntityTree(Of P), T, Class})(
																						  ...
																						  Optional treeUpdate As Func(Of P, DataTree, DataTree) = Nothing
																						 ) As IActionResult
			Dim vm = CreateVMList()
			Dim id As Long? = If(parentId.IsEmpty, Nothing, parentId.ToLong)
			Dim ret = vm.QueryDataTree(Of P)(id, nameField, disabledField, extField, queryVM, treeUpdate)
		...
		''' <param name="nameFields">查询字段</param>
		''' <param name="disabledField">是否禁用字段</param>
		''' <param name="nameUpdate">名称更新函数</param>
		Protected Overridable Function QuerySearch(
													keywords As String,
													Optional queryVM As Q = Nothing,
			...
			Dim vm = CreateVMList()
			Dim ret = vm.QuerySearch(keywords, queryVM, nameFields, disabledField, nameUpdate)
