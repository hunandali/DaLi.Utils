...
Namespace Model
	...
	Public Class DuplicatedExpression(Of T As {IEntity, Class})
		...
		Public Function MakeQuery(db As IFreeSql, entity As T) As (Query As ISelect(Of T), Name As String)
			If db Is Nothing OrElse entity Is Nothing OrElse Not HasExpression Then Return Nothing

			' 已经添加全局过滤器，无需 HiddenFilter
			Dim query = db.Select(entity.GetType).WhereNotEquals(entity.ID, Function(x) x.ID)
			'Dim query = db.Select(Of T).HiddenFilter.WhereNotEquals(obj.ID, Function(x) x.ID)
			Dim names As New List(Of String)

			For Each exp In _Instance
				Dim value = exp.Compile.Invoke(entity)
				If value Is Nothing Then Continue For

				' 当检查表达式项目只有一条，且当前对象的值：文本为空则不进行检测
				If _Instance.Count = 1 AndAlso value.GetType.GetTypeCode = TypeCode.String AndAlso value.ToString.IsEmpty Then Continue For
				...
				If name.NotEmpty AndAlso Not names.Contains(name) Then
					...
					' 对于 nullabled 的处理
					Dim equExp = If(e.Operand.Type.IsNullable, Expression.PropertyOrField(e.Operand, "Value"), e.Operand)
					'If equExp.Type.IsNullable Then equExp = Expression.PropertyOrField(equExp, "Value")

					Dim equal = Expression.Equal(equExp, Expression.Constant(value))
