			...
			Public Function Send(notifier As NotifierInterfaceEnum, user As String, Optional ip As String = "", Optional ByRef errorMessage As String = "") As Boolean
				...
				If cacheValue IsNot Nothing Then
					...
					If cacheValue.Count >= setting.Captcha_Count Then
						Dim time = Math.Ceiling(cacheValue.Last.AddMinutes(setting.Captcha_Count_Delay).Subtract(DATE_NOW).TotalMinutes)
						If time > 0 Then
					...
					If last > DATE_FULL_NOW Then
						Dim time = Math.Ceiling(last.Subtract(DATE_NOW).TotalMinutes)
				...
				End Select
				content = content.EmptyValue("你当前的验证码为：{code}，有效期为 {timeout} 分钟，请勿泄露。").FormatTemplate(Nvs, True)
