...
#Region "公共参数"
		...
		''' <summary>JWT 签名证书</summary>
		Private ReadOnly Property SigningCredentials As SigningCredentials
			Get
				If _SigningCredentials Is Nothing Then
					Dim Key = New SymmetricSecurityKey(Text.Encoding.UTF8.GetBytes(_Setting.SecurityKey))
		...
		''' <summary>JWT 验证参数</summary>
		Private ReadOnly Property ValidationParameters As TokenValidationParameters
			Get
				If _ValidationParameters Is Nothing Then
					_ValidationParameters = New TokenValidationParameters

					' 是否验证 Issuer
					If _Setting.Issuer.IsEmpty Then
						_ValidationParameters.ValidateIssuer = False
					Else
						_ValidationParameters.ValidateIssuer = True
						_ValidationParameters.ValidIssuer = _Setting.Issuer
					End If

					' 是否验证 Audience
					If _Setting.Audience.IsEmpty Then
						_ValidationParameters.ValidateAudience = False
					Else
						_ValidationParameters.ValidateAudience = True
						_ValidationParameters.ValidAudience = _Setting.Audience
					End If

					' 是否验证 SecurityKey
					_ValidationParameters.ValidateIssuerSigningKey = True
					_ValidationParameters.IssuerSigningKey = New SymmetricSecurityKey(Text.Encoding.UTF8.GetBytes(_Setting.SecurityKey))
		...
		Public Sub New()
			...
			_Setting = SYS.GetSetting(Of IJWTSetting)
...
#Region "创建 Token"
		...
		Public Function CreateToken(values As NameValueDictionary, Optional expired As Integer = 0) As String
			...
			If expired < 1 Then expired = _Setting.Expires
			If expired < 1 Then expired = 1

			Return _Handler.CreateEncodedJwt(New SecurityTokenDescriptor With {
				.Issuer = _Setting.Issuer,
				.Audience = _Setting.Audience,
				.Subject = claimsIdentity,
				.NotBefore = DATE_NOW.AddMinutes(-1),
				.Expires = DATE_NOW.AddMinutes(expired),
				.IssuedAt = DATE_NOW,
...
#Region "分析 Token"
		...
		Public Function GetClaims(strToken As String) As NameValueDictionary
			If strToken.NotEmpty Then
				Try
					Dim validate = _Handler.ValidateToken(strToken, ValidationParameters, Nothing)
...
#Region "清除 Token"
		...
		Public Shared Sub ClearToken(response As HttpResponse)
