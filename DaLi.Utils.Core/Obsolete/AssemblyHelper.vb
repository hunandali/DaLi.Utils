...
Namespace Helper
	...
	<Obsolete("请使用 ReflectionHeler 替换")>
...
#Region "加载 Assembly"
		...
		Public Shared Function CreateInstance(filepath As String, fullName As String) As Object
			If filepath.NotEmpty AndAlso fullName.NotEmpty Then
		...
		Public Shared Function Assemblies(Optional includeBin As Boolean = False, Optional pluginFolder As String = "plugins", Optional skipSystemAssembly As Boolean = True) As ImmutableList(Of Assembly)
			'Return AppDomain.CurrentDomain.GetAssemblies.Where(Function(x) IsInvalidName(x.FullName, True)).ToImmutableList
			' .net core 使用 AssemblyLoadContext
			'Return AssemblyLoadContext.Default.Assemblies.Where(Function(x) IsInvalidName(x.FullName, True)).ToImmutableList
			...
			If includeBin Then
				...
				Dim files = IO.Directory.GetFiles(Path, "*.dll", IO.SearchOption.AllDirectories)
				Dim assBin = files.
					Select(Function(x)
							   Try
		...
		''' <param name="isPulic">是否获取此程序集中定义的公共类型的集合，这些公共类型在程序集外可见。</param>
		''' <param name="includeNames">需要包含的 TypeName</param>
		Public Shared Function Load(assembly As Assembly, Optional isPulic As Boolean = False, Optional includeNames As String() = Nothing) As List(Of Object)
			Dim List As New List(Of Object)
