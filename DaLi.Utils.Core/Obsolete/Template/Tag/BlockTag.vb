...
Namespace Template.Tag
	...
	Public Class BlockTag
		...
		Public Sub New(node As String, Optional parent As BlockTag = Nothing)
			...
			' 检查是否有效标签
			If node.IsEmpty Then Return

			' 只允许标签，<ff:xxx></ff:xxx>
			If node.StartsWith("<" & PREFIX, StringComparison.OrdinalIgnoreCase) AndAlso node.EndsWith(">"c) Then node = node.Cut("<" & PREFIX, ">", False, False)

			' 非有效标签，退出检测
			If node.IsEmpty Then Return

			node &= " "
			Dim Len = node.IndexOf(" "c)
		...
		Public Shared Function GetTags(template As String, name As String, Optional defNames As String() = Nothing) As List(Of BlockTag)
			...
			Else
				Return GetTags(template, False, defNames)?.Where(Function(x) x.Name.Equals(name, StringComparison.OrdinalIgnoreCase)).ToList
			End If
		End Function
		...
		Private Shared Sub GetTags(ByRef list As List(Of BlockTag), template As String, Optional defNames As String() = Nothing, Optional allLevel As Boolean = True, Optional parent As BlockTag = Nothing)
			...
			' 默认其实标签
			Dim def = "<" & PREFIX
			If template.IndexOf(def, StringComparison.OrdinalIgnoreCase) < 0 Then Return
			...
			While s > -1 AndAlso s < len
				...
				If Not name.EndsWith("/"c) Then
					Dim spaceStart = name.IndexOf(" "c)
					If spaceStart > 0 Then name = name.Substring(0, spaceStart)
					'name = name.Split(" "c)(0)

					' 分析到 ff:xxx
					If name.NotNull AndAlso name.Length > def.Length Then

						'如果存在 defName 则还需要分析两者名字是否一致
						If defNames?.Any(Function(x) name.Equals($"{PREFIX}{x}", StringComparison.OrdinalIgnoreCase)) Then
							' If defNames Is Nothing OrElse defNames.Length < 1 OrElse defNames.Any(Function(x) name.ToLowerInvariant = (PREFIX & x).ToLowerInvariant) Then
							' 分析结尾
							Dim fin = "</" & name & ">"
							Dim node = GetTag(template, "<" & name, fin, s, e)

							If node.NotNull Then
								Dim Tag = New BlockTag(node, parent)
								If Tag IsNot Nothing AndAlso Tag.Name.NotNull Then
									list.Add(Tag)
