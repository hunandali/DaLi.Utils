...
#End Region
		...
		Protected Sub GetAttributes(strAttrs As String)
			...
			For Each Match As Match In MatchCollection
				If Attributes(Match.Groups(1).Value).IsNull Then Attributes(Match.Groups(1).Value) = Match.Groups(3).Value.DecodeHtml
			Next
		End Sub
		...
		Public Shared Function AttributesReplace(value As String, attrs As NameValueDictionary, Optional replaceFunc As Func(Of NameValueDictionary, String, String） = Nothing) As String
			If attrs?.Count > 0 AndAlso value.NotNull Then
				CHARSET = If(CHARSET, Text.Encoding.UTF8)

				'清理 Html
				Dim strClear = attrs("clear")
				If strClear.NotNull Then
					Select Case strClear.ToLowerInvariant
						Case "trim"
							If value.NotNull Then value = value.Trim
				...
				Dim strTo = attrs("to")
				If strReplace.NotNull Then
					If strReplace.StartsWith("("c) AndAlso strReplace.EndsWith(")"c) Then strReplace = strReplace.Replace("<", "{").Replace(">", "}")
				...
				If strFrom.NotNull Then
					...
					If Ls.NotEmpty Then
						...
						Dim Ret = ""
						If Nvs.TryGetValue(value, Ret) Then value = Ret
				...
				If strDate.NotNull Then
					Dim Dt = value.ToDateTime(DATE_2000)
				...
				If strNow.NotEmpty Then value = ReplaceDate(value, strNow.ToDateTime(DATE_NOW), attrs)

				'格式化，可以加随机码干扰，对于数字会自动加上这个值，对于文字则直接附加此数据
				Dim strFormat = attrs("format")
				If strFormat.NotEmpty Then
					Try
						Select Case strFormat.ToLowerInvariant
							Case "date"
				...
				If strEncode.NotEmpty Then
					Select Case strEncode.Trim.ToLowerInvariant
				...
				If strDecode.NotEmpty Then
					Select Case strDecode.ToLowerInvariant
						Case "js"
							value = value.DecodeJs

						Case "url"
							If value.NotNull Then value = System.Web.HttpUtility.UrlDecode(value, CHARSET)
...
#Region "判断"
				...
				If strValidate.NotEmpty Then
					Select Case strValidate.ToLowerInvariant
						Case "empty"
							value = value.IsEmpty
