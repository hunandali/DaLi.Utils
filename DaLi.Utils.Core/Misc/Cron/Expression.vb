...
#Region "更新"
		...
		Public Function UpdateExpression(exp As String, Optional onlyDay As Boolean = False, Optional ByRef message As String = "") As String
			...
			' 秒
			_Exp(0) = UpdateWildcard(exps(0), 0, 59, Nothing, True)

			' 分
			_Exp(1) = UpdateWildcard(exps(1), 0, 59, Nothing, True)

			' 时
			_Exp(2) = UpdateWildcard(exps(2), 0, 23, Nothing, True)
			...
			' 月
			_Exp(4) = UpdateWildcard(exps(4), 1, 12, Nothing, True)

			' 周
			_Exp(5) = exps(5).EmptyValue("*")

			' 年
			_Exp(6) = UpdateWildcard(exps(6), 2020, 3000, Nothing, True)
		...
		''' <param name="forceChange">对没有更新过的进行强制更新为 *，防止月，年等字段出现文本</param>
		''' <remarks>
		''' 逗号（,） 指定数字集合。 1,4,7 代表：1、4、7
		''' 横线（-） 代表区间，一段数字集合。 3-6 代表：3、4、5、6
		''' 斜线（/） 间隔数据，从第一个数字开始，每隔第二个数字重复（0为无效间隔，负数则倒计数）。  3/5 从3开始，每隔5：3、8、13、18……；55/-10 表示从55开始倒数，含55、45、35、25、15、5
		''' 星号（*） 忽略此值，此段值不限
		'''</remarks>
		Private Shared Function UpdateWildcard(s As String, min As Integer, max As Integer, Optional ByRef hasChange As Boolean = False, Optional forceChange As Boolean = False) As String
			hasChange = True
			If s.IsEmpty OrElse s = "*" Then Return "*"

			' 逗号（,）
			If s.Contains(","c) Then
				Dim vals = s.ToIntegerArray(True)
		...
		Private Shared Function UpdateDay(exp As String) As String
			...
			If exp.EndsWith("R", StringComparison.OrdinalIgnoreCase) Then
				Select Case exp.ToUpperInvariant
					Case "R", "FR", "BR", "ER", "LR"
						Return exp.ToUpperInvariant
					Case Else
						Return "R"
				End Select
			End If
		...
		Private Shared Function GetWorkWildcard(exp As String) As Integer?
			...
			If exp.EndsWith("W", StringComparison.OrdinalIgnoreCase) Then
				If exp.Length > 1 Then
					...
					Select Case exp.ToLowerInvariant
						Case "l"
							R = -1
						Case "f"
							R = 1
