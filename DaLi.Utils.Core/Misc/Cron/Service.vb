...
Namespace Misc.Cron
	...
	Public NotInheritable Class Service
							 ...
							 Parallel.ForEach(_Instance.Keys,
											  ...
											  Sub(key)
												  ...
												  If Not Value.TimeUp Then
													  ' 时间未到的需要检查，一旦成功则不再检查
													  If Expression.Timeup(Value.Exps, DATE_NOW, Value.Last, Value.onlyDay) Then
														  Value.TimeUp = True
														  Value.Last = DATE_NOW
														  _Instance(key) = Value

														  'CON.Warn({key, Value.Exps.JoinString, DATE_NOW, Value.Last})
													  End If
												  End If
											  End Sub)
		...
		Public Shared Sub Register(id As String, exp As String, Optional onlyDay As Boolean = False)
			...
			' 记录数据
			Dim val As (Exps As String(), Last As Date, TimeUp As Boolean, onlyDay As Boolean) = Nothing
			If _Instance.TryGetValue(id, val) Then
				val.Exps = exps
				val.onlyDay = onlyDay
				val.TimeUp = False
			...
			'	_Instance(id) = val
			'Else
			'	_Instance.TryAdd(id, (exps, Date.MinValue, False, onlyDay))
			'End If
		End Sub
		...
		Public Shared Function TimeUp(id As String, Optional lastTime As Date = Nothing) As Boolean
			...
			If _Instance.TryGetValue(id, val) Then
				...
				isUp = val.TimeUp
				val.Last = If(lastTime.IsValidate, lastTime, DATE_NOW)
				val.TimeUp = False
		...
		Public Shared Function Item(id As String) As (Exps As String, Last As Date, TimeUp As Boolean, onlyDay As Boolean)
			Dim ret As (Exps As String(), Last As Date, TimeUp As Boolean, onlyDay As Boolean) = Nothing
			If id.NotEmpty AndAlso _Instance.TryGetValue(id, ret) Then
