...
#Region "加解密"
		...
		Public Function Encrypt(source As Byte(), encryptPublicKey As String) As Byte()
			...
			Try
				If source?.Length > 0 AndAlso encryptPublicKey.NotNull Then
					Using Rsa = CreateRsa()
		...
		Public Function Encrypt(source As String, encryptPublicKey As String) As String
			...
			If source.NotNull AndAlso encryptPublicKey.NotNull Then
		...
		Public Function Decrypt(source As Byte(), decryptPrivateKey As String) As Byte()
			...
			If source?.Length > 0 AndAlso decryptPrivateKey.NotNull Then
				Using Rsa = CreateRsa()
		...
		Public Function Decrypt(source As String, decryptPrivateKey As String) As String
			...
			If source.NotNull AndAlso decryptPrivateKey.NotNull Then
...
#Region "私匙/公钥"
		...
		Public Function IsKey(privateKey As String) As Boolean
			...
			If privateKey.NotEmpty Then
				Try
					R = PublicKey(privateKey).NotEmpty
				Catch ex As Exception
				End Try
			End If
...
#Region "加签/验签"
		...
		Public Function Sign(source As String, signPriveteKey As String) As String
			...
			If source.NotNull AndAlso signPriveteKey.NotNull Then
				Using Rsa = CreateRsa()
		...
		Public Function Sign(source As Byte(), signPriveteKey As String) As Byte()
			...
			If source.NotEmpty AndAlso signPriveteKey.NotNull Then
				Using Rsa = CreateRsa()
					FromXml(Rsa, signPriveteKey)
		...
		Public Function Sign(source As IO.Stream, signPriveteKey As String) As Byte()
			...
			If source IsNot Nothing AndAlso signPriveteKey.NotNull Then
				Using Rsa = CreateRsa()
					FromXml(Rsa, signPriveteKey)
					R = Rsa.SignData(source, HashAlgorithm, SignaturePadding)
				End Using
		...
		Public Function Verify(source As String, signature As String, verifyPublicKey As String) As Boolean
			If source.NotNull AndAlso signature.NotNull AndAlso verifyPublicKey.NotNull Then
		...
		Public Function Verify(source As Byte(), signature As Byte(), verifyPublicKey As String) As Boolean
			...
			If source.NotEmpty AndAlso signature.NotEmpty AndAlso verifyPublicKey.NotNull Then
				Using Rsa = CreateRsa()
		...
		Public Function Verify(source As IO.Stream, signature As Byte(), verifyPublicKey As String) As Boolean
			...
			If source IsNot Nothing AndAlso signature.NotEmpty AndAlso verifyPublicKey.NotNull Then
				Using Rsa = CreateRsa()
					FromXml(Rsa, verifyPublicKey)
					R = Rsa.VerifyData(source, signature, HashAlgorithm, SignaturePadding)
				End Using
		...
		Public Function VerifyWithSHA256(source As String, signature As String, verifyPublicKey As String) As Boolean
			...
			Dim keyData = ConvertFromPublicKey(verifyPublicKey)
...
#If NET8_0 Then
			Using aesGcm As New AesGcm(byteKey, 16)
#Else
...
#End If
	...
	Public NotInheritable Class Sign
			...
			If key <> Guid.Empty Then
				Select Case mode
					...
					Case ClientSignEnum.PASSWORD
						' HASH: MD5（账号+密码+请求方式+地址）

						method = method.EmptyValue.ToUpperInvariant
						path = path.EmptyValue.ToLowerInvariant

						Sign = MakeSign(account, key, appId, method, path)
					...
					Case ClientSignEnum.SIGN
						...
						Dim time = Math.Floor(DATE_NOW.UnixTicks / 300)
					...
					Case ClientSignEnum.SIGN_RND
						...
						Dim time = Math.Floor(DATE_NOW.UnixTicks / 600)
					...
					Case ClientSignEnum.SIGN_TIME
						...
						Dim time = If(ticks < 1, DATE_NOW.JsTicks, ticks)
		...
		Public Shared Function ParameterSortString(parameters As Object) As String
			If parameters Is Nothing Then Return ""

			Dim Nvs As NameValueDictionary

			' 非文本字典先转换成文本字典
			If parameters.GetType.IsComeFrom(Of NameValueDictionary) Then
				Nvs = TryCast(parameters, NameValueDictionary)
			Else
				Nvs = JsonExtension.ToJson(parameters).ToJsonNameValues
			End If

			If Nvs.IsEmpty Then Return ""

			' 处理非空值数据
			Dim ret = Nvs.Where(Function(x) x.Value.NotEmpty).
				Select(Function(x) $"{x.Key}={TypeExtension.ToObjectString(x.Value)}".ToLowerInvariant). ' 转换小写
				OrderBy(Function(x) x, StringComparer.Ordinal). ' 排序，注意原数据已经转换成小写，排序使用忽略大小写，否则会导致大写在前的情况
				JoinString("&")
