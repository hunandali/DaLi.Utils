...
Public Class LRU(Of K, V)
	...
	Public Sub Put(key As K, value As V)
		If Size < 1 OrElse key Is Nothing Then Return
	...
	Private Sub Update(node As Node)
		...
		' 只有一个数据的时候，无需调整
		If Instance.Count < 2 Then Return

		' 检查是否队首节点，队首则不处理 
		If node.Key.Equals(_First) Then Return

		SyncLock Instance
			' 原位置前后对接
			If node.Prv IsNot Nothing Then
				Dim tmp = Nothing
				If Instance.TryGetValue(node.Prv, tmp) AndAlso tmp IsNot Nothing Then tmp.Nxt = node.Nxt

				'Dim tmp = If(Instance.ContainsKey(node.Prv), Instance(node.Prv), Nothing)
				'If tmp IsNot Nothing Then tmp.Nxt = node.Nxt
			End If
			If node.Nxt IsNot Nothing Then
				Dim tmp = Nothing
				If Instance.TryGetValue(node.Nxt, tmp) AndAlso tmp IsNot Nothing Then tmp.Prv = node.Prv

				'Dim tmp = If(Instance.ContainsKey(node.Nxt), Instance(node.Nxt), Nothing)
