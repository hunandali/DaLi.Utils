...
#End Region
			...
			If Request.Cookies IsNot Nothing Then
				...
				If Req.RequestUri IsNot Nothing Then
					Host = Req.RequestUri.Host.ToLowerInvariant
				End If
			...
			' 请求方式
			Req.Method = Request.Method.Name.ToUpperInvariant
		...
		Public Sub ExecuteAsync(callBack As Action(Of HttpWebResponse), Optional cancelAction As Func(Of Boolean) = Nothing)
			' 防止重复请求，上次操作未完成不再操作
			If _Status <> Model.HttpStatusEnum.UNKNOWN AndAlso _Status <> Model.HttpStatusEnum.FINISH Then Return
