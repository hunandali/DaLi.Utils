...
Namespace Extension
	...
	Public Module ModelExtension
		...
		Public Function ToDeepDictionary(ByRef this As IDictionary(Of String, Object)) As Dictionary(Of String, Object)
			...
			For Each Key In this.Keys
				...
				If P > -1 Then
					...
					Dim BValue = Nothing
					If Not Ret.TryGetValue(Bkey, BValue) Then Ret.Add(Bkey, New Dictionary(Of String, Object))

					Dim d = If(TryCast(BValue, Dictionary(Of String, Object)), New Dictionary(Of String, Object))
					d.TryAdd(Ekey, Value)
		...
		<Extension>
		Public Function ToSingleDictionary(Of T)(this As IDictionary(Of String, T), Optional keepSource As Boolean = False) As Dictionary(Of String, Object)
			Dim Ret As New KeyValueDictionary
			If this Is Nothing OrElse this.Count < 1 Then Return Ret
