...
Namespace Extension
	...
	Public Module TypeExtension
		...
		''' </remarks>
		<Extension>
		<Obsolete("请使用 IsEmptyValue 代替")>
		Public Function NoValue(Of T)(this As T, Optional validate As Func(Of T, Boolean) = Nothing) As Boolean
			If this Is Nothing Then Return True
			If "".Equals(this) OrElse 0.Equals(this) OrElse False.Equals(this) OrElse Date.MinValue.Equals(this) OrElse Guid.Empty.Equals(this) Then Return True

			Dim type = this.GetType

			' 是否 ICollection，包含 List, Dictionary, Array 等
			If type.IsCollection Then Return TryCast(this, ICollection)?.Count = 0

			' 数组
			If type.IsArray Then Return TryCast(this, Array)?.Length = 0
		...
		<Extension>
		<Obsolete("请使用 IsEmptyValue 代替")>
		Public Function NoValue(this As Object, Optional validate As Func(Of Object, Boolean) = Nothing) As Boolean
			Return NoValue(Of Object)(this, validate)
		End Function
...
#Region "类型判断"
		...
		Public Function IsNullable(this As Type) As Boolean
			...
			Return Nullable.GetUnderlyingType(this) IsNot Nothing
		End Function

		''' <summary>判断是否为指定类型或这 Nullable 指定类型</summary>
		''' <param name="innerType">是否为指定类型</param>
		''' <param name="this">当前类型</param>
		<Extension>
		Public Function IsNullable(this As Type, innerType As Type) As Boolean
			If this Is Nothing Then Return False
			If innerType Is Nothing Then Return IsNullable(this)

			If this = innerType Then Return True

			Dim underlyingType = Nullable.GetUnderlyingType(this)
			Return underlyingType IsNot Nothing AndAlso underlyingType = innerType
		End Function

		''' <summary>判断是否为指定类型或这 Nullable 指定类型</summary>
		<Extension>
		Public Function IsNullable(Of T)(this As Type) As Boolean
			Return IsNullable(this, GetType(T))
		End Function

		''' <summary>判断是否为指定类型或这 Nullable 指定类型</summary>
		''' <param name="this">当前类型</param>
		<Extension>
		Public Function IsNullable(this As Type, innerTypeCheck As Func(Of Type, Boolean)) As Boolean
			If this Is Nothing Then Return False
			If innerTypeCheck Is Nothing Then Return IsNullable(this)

			If innerTypeCheck(this) Then Return True

			Dim underlyingType = Nullable.GetUnderlyingType(this)
			Return underlyingType IsNot Nothing AndAlso innerTypeCheck(underlyingType)
		End Function
		...
		''' <summary>判断是否为 ICollection 类型</summary>
		<Extension>
		Public Function IsCollection(this As Type) As Boolean
			Return GetType(ICollection).IsAssignableFrom(this)
		End Function

		''' <summary>判断是否为 IDictionary 类型，注意 IEnumerable, ICollection 包含 IDictionary，IEnumerable, ICollection 是 IDictionary 基类</summary>
		<Extension>
		...
		Public Function IsNullableEnum(this As Type) As Boolean
			Return this.IsNullable(Function(x) x.IsEnum)
		...
		Public Function IsNullableNumber(this As Type) As Boolean
			Return this.IsNullable(Function(x) x.IsNumber)
		...
		Public Function IsNullableGuid(this As Type) As Boolean
			Return this.IsNullable(Of Guid)
		...
		Public Function IsNullableInteger(this As Type) As Boolean
			Return this.IsNullable(Of Integer)
		...
		Public Function IsNullableLong(this As Type) As Boolean
			Return this.IsNullable(Of Long)
		...
		Public Function IsNullableBoolean(this As Type) As Boolean
			Return this.IsNullable(Of Boolean)
		...
		Public Function IsNullableDate(this As Type) As Boolean
			Return this.IsNullable(Of Date)
		...
		'	' 排除系统类型
		'	If this.IsPrimitive OrElse
		'	   this.IsValueType OrElse
		'	   this Is GetType(String) OrElse
		'	   this Is GetType(Object) OrElse
		'	   this.IsArray OrElse
		'	   this.IsEnum OrElse
		'	   this.IsInterface OrElse
		'	   this.IsAbstract Then Return False

		'	' 检查命名空间和程序集
		'	Return this.IsClass AndAlso
		'		Not this.Assembly.Equals(GetType(String).Assembly) AndAlso
		'		Not this.Namespace?.StartsWith("System") AndAlso
		'		Not this.Namespace?.StartsWith("Microsoft")
		'End Function
...
#Region "转换"
		...
		''' <summary>获取对象全名称</summary>
		''' <param name="this">类型</param>
		<Extension>
		Public Function GetTypeFullName(this As Object) As String
		...
		''' <param name="this">类型</param>
		<Extension>
		<Obsolete("请使用 GetDefaultValue 代替")>
		...
		Public Function ToObjectString(this As Object) As String
			...
			ElseIf thisCode = TypeCode.Object Then
				...
				Select Case thisType
					...
					Case Else
						Return Json.JsonExtension.ToJson(this, False, False, False)
				End Select
			End If
...
#Region "自定义字段类型 FieldType"
		...
		Public Function DefaultValue(this As FieldTypeEnum) As Object
			Select Case this
				...
				Case FieldTypeEnum.TIME
					Return Date.MinValue.ToString("HH:mm:ss")

				Case FieldTypeEnum.DATE
					Return Date.MinValue.ToString("yyyy-MM-dd")
		...
		Public Function DefaultArrayValue(this As FieldTypeEnum) As Array
			Select Case this
				...
				Case FieldTypeEnum.DICTIONARY
					Return Array.Empty(Of IDictionary(Of String, Object))
			...
			If this IsNot Nothing Then
				Select Case type
					...
					Case FieldTypeEnum.LOWER_CASE
						Ret = this.ToString.ToLowerInvariant

					Case FieldTypeEnum.UPPER_CASE
						Ret = this.ToString.ToUpperInvariant
					...
					Case FieldTypeEnum.JSON, FieldTypeEnum.DICTIONARY
						Ret = Json.JsonExtension.ToJson(this, False, False, False)
		...
		Public Function ToValue(this As String, type As FieldTypeEnum, Optional validate As Func(Of Object, Boolean) = Nothing, Optional converAll As Boolean = False) As Object
			...
			Select Case type
				...
				Case FieldTypeEnum.DATE
					Dim v = this.ToDateTime
					If v > Date.MinValue Then
				...
				Case FieldTypeEnum.TIME
					Ret = ("2000-01-01 " & this).ToDateTime.ToString("HH:mm:ss")

				Case FieldTypeEnum.UPPER_CASE
					Ret = this.ToUpperInvariant

				Case FieldTypeEnum.LOWER_CASE
					Ret = this.ToLowerInvariant
				...
				Case FieldTypeEnum.FOLDER
					...
					Dim startWithLine = (this.StartsWith("/"c) OrElse this.StartsWith("\"c)) AndAlso Not this.StartsWith("//")
		...
		Public Function ChangeType(Of T)(this As Object) As T
			...
			Catch e1 As Exception
				' 直接转换无效，尝试 JSON 序列化
				Try
					Return Json.JsonExtension.ToJson(this, False, False, False).FromJson(Of T)
