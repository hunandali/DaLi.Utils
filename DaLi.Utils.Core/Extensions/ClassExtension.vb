			...
			proSource.
				...
				ForEach(Sub(x)
							...
							If pro IsNot Nothing Then
								' 强制替换
								Dim targetValue = pro.GetValue(target)
								If replaceNothing Then x.SetValue(this, targetValue)

								' 目标值为空，不处理
								If targetValue Is Nothing Then Return

								' 新旧值一致，不处理
								Dim sourceValue = x.GetValue(this)
								If targetValue.Equals(sourceValue) Then Return

								' 对于目标值为默认值的情况，也不进行处理，例如 GUID.Empty
								Dim defaultValue = pro.PropertyType.GetDefaultValue
								If Not targetValue.Equals(defaultValue) Then x.SetValue(this, targetValue)
