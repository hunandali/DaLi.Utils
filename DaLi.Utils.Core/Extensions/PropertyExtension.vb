...
#Region "获取属性列表"
		...
		'	Dim pros As PropertyInfo() = Nothing

		'	SyncLock _PropertyCache
		'		If Not _PropertyCache.TryGetValue(name, pros) Then
		'			pros = this.GetProperties(BindingFlags.Instance Or BindingFlags.Static Or BindingFlags.Public Or BindingFlags.NonPublic)
...
#Region "获取公共属性列表"
		...
		Public Function GetOutputFields(this As Type, returnSimple As Boolean) As ImmutableDictionary(Of PropertyInfo, Integer)
			...
			SyncLock _LockObject
				If Not _OutputFieldCache.TryGetValue(cacheName, outputFields) Then
					' 1. 获取默认属性的参数
					...
					If pros.NotEmpty Then
									...
									For Each pro In pros.Keys
										If pros(pro) = -1 Then
											For Each inPro In inPros
												...
												If inPro.TryGetValue(pro.Name, inValue) Then
													'hasReplace = True
													pros(pro) = inValue
													Exit For
												End If
						...
						' 移除相同名称的字段，移除掉基类属性
						Dim Names = pros.
							Select(Function(x) x.Key.Name.ToLowerInvariant).
						...
						If Names?.Count > 0 Then
							Names.ForEach(Sub(item)
											  ' 获取所有相关类
											  Dim keys = pros.Where(Function(x) x.Key.Name.Equals(item, StringComparison.OrdinalIgnoreCase)).Select(Function(x) x.Key).ToList
		...
		'				Names.ForEach(Sub(item)
		'								  ' 获取所有相关类
		'								  Dim keys = fields.Where(Function(x) x.Key.Name.ToLowerInvariant = item).Select(Function(x) x.Key).ToList
		'								  keys.ForEach(Sub(key)
		'												   Dim type = key.DeclaringType
