...
Namespace Extension
	...
	Public Module EnumerableExtension
		...
		<Extension()>
		Public Function GetHash(Of T)(this As IList(Of T)) As String
			If this.IsEmpty Then Return ""
			Return this.Select(Function(x) x.ToObjectString).OrderBy(Function(x) x).JoinString(vbCrLf).MD5
		End Function

		''' <summary>生成 Hash</summary>
		<Extension()>
		Public Function GetHash(Of T)(this As ICollection(Of T)) As String
			If this.IsEmpty Then Return ""
			Return this.Select(Function(x) x.ToObjectString).OrderBy(Function(x) x).JoinString(vbCrLf).MD5
		End Function
		...
		''' <summary>生成 Hash</summary>
		<Extension()>
		Public Function GetHash(Of K, V)(this As IDictionary(Of K, V)) As String
			If this.IsEmpty Then Return ""
			Return this.Select(Function(x) $"{x.Key.ToObjectString}=>{x.Value.ToObjectString}").OrderBy(Function(x) x).JoinString(vbCrLf).MD5
		End Function

		''' <summary>生成 Hash</summary>
		<Extension()>
		Public Function GetHash(Of T)(this As IDictionary(Of String, T), Optional skipCase As Boolean = True) As String
			If this.IsEmpty Then Return ""
			Return this.Select(Function(x) $"{If(skipCase, x.Key.ToLowerInvariant, x.Key)}=>{x.Value.ToObjectString}").OrderBy(Function(x) x).JoinString(vbCrLf).MD5
		End Function
