...
Namespace Extension
	...
	Public Module ArrayExtension
		...
		Public ReadOnly SEPARATOR_DEFAULT_BASE As Char() = {","c, ";"c, vbLf}

		''' <summary>默认分隔符，包含全半角回车</summary>
		Public ReadOnly SEPARATOR_DEFAULT As String() = {","c, "，"c, ";"c, "；"c, vbCr, vbLf}
...
#Region "1. 字符转数组"
		...
		Public Function SplitEx(this As String, Optional separator As String() = Nothing, Optional splitOption As SplitEnum = SplitEnum.NONE) As String()
			...
			' 数据无效
			If isTrim AndAlso this.IsEmpty Then Return Nothing
...
#Region "2. 字符转数值数组"
		...
		<Extension>
		Public Function ToIntegerArray(this As String, Optional includeZero As Boolean = False) As Integer()
			...
			If Arrs.IsEmpty Then Return Array.Empty(Of Integer)

			Return Arrs.Select(Function(x) x.ToInteger(False)).Where(Function(x) includeZero OrElse x <> 0).Distinct.ToArray
		End Function

		''' <summary>字符串转换成数据列表</summary>
		''' <param name="includeZero">是否包含零</param>
		<Extension>
		Public Function ToLongArray(this As String, Optional includeZero As Boolean = False) As Long()
			Dim Arrs = this.SplitEx(String.Empty, SplitEnum.REMOVE_SAME Or SplitEnum.REMOVE_EMPTY_ENTRIES Or SplitEnum.RETRUN_DBC)
			If Arrs.IsEmpty Then Return Array.Empty(Of Long)

			Return Arrs.Select(Function(x) x.ToLong(False)).Where(Function(x) includeZero OrElse x <> 0).Distinct.ToArray
		End Function
...
#Region "4. 判断"
		...
		''' <param name="this">数组</param>
		<Extension>
		Public Function EmptyValue(this As IEnumerable, defaultValues As IEnumerable) As IEnumerable
			If this.IsEmpty Then
				Return defaultValues
			Else
				Return this
