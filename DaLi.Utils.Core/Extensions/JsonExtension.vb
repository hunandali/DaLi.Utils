...
Namespace Extension
	...
	Public Module JsonExtension
		...
		'Public ReadOnly JsonLowerCaseNamingPolicy = New Misc.Json.JsonLowerCaseNamingPolicy

		'''' <summary>大写键规则</summary>
		'Public ReadOnly JsonUpperCaseNamingPolicy = New Misc.Json.JsonUpperCaseNamingPolicy

		'#Region "Json 序列化"

		'		Private _JsonOptions As Dictionary(Of String, JsonSerializerOptions)

		'		Private Function GetOption(Optional indented As Boolean = True, Optional camelCase As Boolean = True, Optional skipNull As Boolean = False) As JsonSerializerOptions
		'			If _JsonOptions Is Nothing Then _JsonOptions = New Dictionary(Of String, JsonSerializerOptions)
		'			Dim key = $"{If(indented, 1, 0)}{If(camelCase, 1, 0)}{If(skipNull, 1, 0)}"
		'			Dim value As JsonSerializerOptions = Nothing

		'			If _JsonOptions.TryGetValue(key, value) Then Return value

		'			value = New JsonSerializerOptions With {
		...
		'			If this IsNot Nothing Then
		'				Dim options = GetOption(indented, camelCase, skipNull)
		...
		Public Function JsonElementParse(this As JsonElement, Optional removeNothing As Boolean = False, Optional replaceTags As IDictionary(Of String, Object) = Nothing) As Object
			Select Case this.ValueKind
				...
				Case JsonValueKind.Object
					Dim Ret = this.EnumerateObject.
						Select(Function(x) (x.Name, JsonElementParse(x.Value, removeNothing, replaceTags))).
						Where(Function(x) x.Item2 IsNot Nothing OrElse Not removeNothing).
						ToDictionary(Function(x) x.Name, Function(x) x.Item2)
				...
				Case JsonValueKind.Array
					Dim Ret = this.EnumerateArray.
						Select(Function(x) JsonElementParse(x, removeNothing, replaceTags)).
						Where(Function(x) x IsNot Nothing OrElse Not removeNothing).
						ToList
				...
				Case JsonValueKind.String
					...
					Return If(replaceTags.IsEmpty, Value, Value.FormatTemplate(replaceTags, False))
