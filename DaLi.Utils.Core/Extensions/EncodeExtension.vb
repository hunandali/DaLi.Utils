...
#Region "其他操作"
		...
		''' <param name="encodeAll">是否编码所有字符，true 所有字符都编码，default 仅编码非ASCII，false 仅编码控制符</param>
		<Extension>
		Public Function EncodeHtml(this As String, Optional encodeAll As TristateEnum = TristateEnum.FALSE) As String
			If encodeAll = TristateEnum.FALSE Then Return Web.HttpUtility.HtmlEncode(this)

			With New StringBuilder
				For Each c In this
					Dim b As Integer = AscW(c)

					If encodeAll = TristateEnum.TRUE OrElse b > 127 Then
						.Append($"&#x{Hex(b)};")
					Else
						.Append(c)
					End If
