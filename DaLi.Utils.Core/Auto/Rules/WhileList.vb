...
#Region "EXECUTE"
		...
		Protected Overrides Function ExecuteRule(data As IDictionary(Of String, Object), message As AutoMessage, cancel As CancellationToken) As Object
			...
			Dim value = AutoHelper.GetVarString(Source, data).ToJsonList(True)
			...
			Dim Max = Skip + Count - 1
			If Max >= value.Count Then Max = value.Count - 1

			' 执行循环
			Return WhileExecute(Skip, Max, 1, data,
								Function(idx) New Dictionary(Of String, Object) From {{"_item", value(idx)}},
								message, cancel)
		End Function
