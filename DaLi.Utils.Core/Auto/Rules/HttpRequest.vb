...
#Region "EXECUTE"
		...
		''' <summary>更新处理规则</summary>
		Protected Function GetRule(data As IDictionary(Of String, Object), message As AutoMessage) As HttpRequest
		...
		'Protected Overloads Function ExecuteRule(rule As HttpRequest, message As AutoMessage, cancel As CancellationToken) As Object
		''' <summary>执行规则</summary>
		Protected Function HttpExecute(rule As HttpRequest, message As AutoMessage) As Object
			...
			Using http As New HttpClient
				...
				' Timeout
				http.Timeout = rule.Timeout.Range(5) * 1000
				...
				' 变量名称，未设置则无需更多处理，设置则记录结果
				If Output.NotEmpty Then
					' 当前规则
					ret.Add("Rule", rule)
					...
					If ReturnMode = 1 OrElse ReturnMode = 2 Then
						...
						Dim obj = If(ReturnMode = 2, content.FromJson, content)
					...
					ret.Add("StatusDescription", http.StatusDescription)

					' Header
					'http.ResponseHeaders?.ForEach(Sub(k, v) ret.Add($"{Output}.Header.{k}", v))
					ret.Add("Headers", http.ResponseHeaders)

					' Cookies
					ret.Add("Cookies", http.GetCookies)
		...
		Protected Overrides Function ExecuteRule(data As IDictionary(Of String, Object), message As AutoMessage, cancel As CancellationToken) As Object
			Return HttpExecute(GetRule(data, message), message)
		End Function
