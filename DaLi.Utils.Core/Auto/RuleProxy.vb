...
#Region "EXECUTE"
		...
		Protected Overrides Function ExecuteRule(data As IDictionary(Of String, Object), message As AutoMessage, cancel As CancellationToken) As Object
			...
			Dim ret = client.ExecuteApi(Of AutoMessage)(Http.Model.HttpMethodEnum.POST, "auto/rule", New With {.Rule = Rule.EncodeBase64, data}.ToJson(False, False, False))
			If ret IsNot Nothing AndAlso ret.Data IsNot Nothing Then
			...
			Else
				message.Message = $"代理规则执行异常 {ret?.Message}"
				Return Nothing
			End If
		End Function
