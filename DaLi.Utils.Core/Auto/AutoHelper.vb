	...
	Public NotInheritable Class AutoHelper
		...
		Public Shared Function SysInfo() As Dictionary(Of String, Object)
			...
			Return New Dictionary(Of String, Object) From {
					...
					{"copyright", Ass.Copyright.EmptyValue($"©{DATE_FULL_NOW.Year} 大沥网络")},
		...
		Public Shared Sub UpdateData(data As SafeKeyValueDictionary, result As IDictionary(Of String, Object))
			...
			SyncLock data
				For Each kv In Kvs
					Dim value = Nothing
					If data.TryGetValue(kv.Key, value) Then
						' 如果值为字典数据，则会与之前的数据合并
						Dim typeOld = value?.GetType.IsDictionary(Of String, Object)
		...
		Public Shared Property ProxyMode As Boolean = False
		...
		Public Shared ReadOnly Property IsDebug(data As IDictionary(Of String, Object)) As Boolean
			Get
				Dim debug = Nothing
				Return data IsNot Nothing AndAlso data.TryGetValue("_DEBUG_", debug) AndAlso debug?.Equals(True)
...
#Region "规则"
		...
		Public Shared Property RuleTypes As ImmutableList(Of Type)
			Get
				If _RuleTypes Is Nothing Then
					...
					For Each Ass In ReflectionHeler.CurrentAssemblies
						Dim Types = ReflectionHeler.GetTypes(Ass, True).Where(Function(x) x.IsClass AndAlso Not x.IsAbstract).ToList
		...
		Public Shared ReadOnly Property RuleItem(rule As String) As IRule
			Get
				...
				If ruleType IsNot Nothing Then
					' 反序列为实际规则
					Return TryCast(rule.FromJson(ruleType), IRule)
		...
		Public Shared Function RuleExecute(rule As String, ByRef data As IDictionary(Of String, Object), Optional cancel As CancellationToken = Nothing) As AutoMessage
			...
			Return item.Execute(data, cancel)
		End Function
...
#Region "流程"
		...
		Public Shared Function FlowExecute(rules As String, Optional fullFlow As Boolean = True, Optional ByRef sourceData As SafeKeyValueDictionary = Nothing, Optional ByRef message As AutoMessage = Nothing, Optional cancel As CancellationToken = Nothing) As IDictionary(Of String, Object)
			Return FlowExecute(RuleList(rules, False), True, fullFlow, sourceData, message, cancel)
		End Function
		...
		''' <param name="message">消息</param>
		Public Shared Function FlowExecute(rules As IEnumerable(Of IRule), Optional validateRule As Boolean = True, Optional fullFlow As Boolean = True, Optional ByRef data As SafeKeyValueDictionary = Nothing, Optional ByRef message As AutoMessage = Nothing, Optional cancel As CancellationToken = Nothing) As IDictionary(Of String, Object)
			...
			flow?.TryGetValue("name", flowName)
			...
			Try
				...
				For Each item In rules
					...
					Dim msg = item.Execute(forData, cancel)
			...
			Catch ex As OperationCanceledException
				' 中断操作，不拦截
				message.SetSuccess(False, $"流程{flowName}完成 已经强制结束")
...
#Region "获取值"
		...
		Public Shared Function GetVar(source As String, data As IDictionary(Of String, Object)) As Object
			...
			If source.StartsWith("<%") AndAlso source.EndsWith("%>") Then
				...
				Dim result = Nothing
				If data.TryGetValue(source, result) Then Return result
				...
				If source.Contains("."c) AndAlso Not source.StartsWith("."c) AndAlso Not source.EndsWith("."c) Then
					' 尝试将数据转换成 JSON 后变成集合
					Dim value = data.ToJson.FromJson
					...
					For I = 0 To max
						...
						Else
							' 字符键
							Dim dic = TryCast(value, Dictionary(Of String, Object))
			...
			' 非文本模板，且非变量，直接返回原值
			Return source.FormatTemplate(data, "{"c, "}"c)
		End Function
		...
		Public Shared Function GetVarObject(source As String, data As IDictionary(Of String, Object)) As Object
			...
			If var.GetType.IsString Then
				Dim obj = var.ToString.FromJson(True, Function(x) GetVar(x, data))
