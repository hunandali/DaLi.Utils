...
Namespace Auto
	...
	Public MustInherit Class RuleBase
		...
		''' <remarks>如：计时器。True 则将计时器的结果与计时器内的模块值都输出到主流程；False 则只返回计时器的结果，则将导致内部数据无法被外部使用</remarks>
		Public Overridable ReadOnly Property OutResult As Boolean Implements IRule.OutResult
			Get
				Return False
			End Get
		End Property
		...
		Protected MustOverride Function ExecuteRule(data As IDictionary(Of String, Object), message As AutoMessage, cancel As CancellationToken) As Object

		''' <summary>执行操作，并返回当前的变量及相关值</summary>
		Public Function Execute(data As IDictionary(Of String, Object), cancel As CancellationToken) As AutoMessage Implements IRule.Execute
			...
			' 执行操作
			Try
				message.Result = ExecuteRule(data, message, cancel)
				...
				If Not message.Success Then
					...
					' 是否忽略失败，未忽略则抛出异常
					If Not EmptyIgnore Then Throw New AutoException(ExceptionEnum.EXECUTE_ERROR, message, message.Message.EmptyValue("规则执行失败"))

					' 标记失败已经被忽略
					message.SetSuccess(False, $"[失败已忽略] {message.Message}")
				Else
					' 需要返回变量值，但是返回空，需要报异常
					If message.Result Is Nothing AndAlso Output.NotEmpty Then
						If EmptyIgnore Then
							' 无内容
							message.Message = $"[无内容已忽略] {message.Message}"
						Else
							Throw New AutoException(ExceptionEnum.NO_RESULT, message)
						End If
					End If
				End If
			...
			Catch ex As Exception
				...
				errMessage = $"{If(ErrorIgnore, "[错误已忽略] ", "")}规则[{Name}]执行异常：{errMessage}"
				message.SetSuccess(ErrorIgnore, $"规则[{Name}]执行异常：" & errMessage)
