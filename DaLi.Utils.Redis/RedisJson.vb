...
#Region "基础操作"
	...
	Private Function GetPath(path As String, Optional ByRef errorMessage As String = "") As String
		...
		errorMessage = ""
		Return If(IgnoreCase, path.ToLowerInvariant, path)
	End Function

	''' <summary>检查获取操作的字段名称</summary>
	Private Function GetPath(paths As String(), Optional ByRef errorMessage As String = "") As String()
		errorMessage = "路径未设置"
		If paths.IsEmpty Then Return Nothing

		errorMessage = "存在异常路径"
		Dim ps = paths.Where(Function(x) x.NotEmpty).Select(Function(x) If(IgnoreCase, x.ToLowerInvariant, x)).ToArray
		If ps.Length <> paths.Length Then Return Nothing
	...
	'	errorMessage = ""
	'	Return If(IgnoreCase, paths.Select(Function(x) x.ToLowerInvariant).ToArray, paths)
	'End Function

	''' <summary>小写键规则</summary>
	Public ReadOnly JsonLowerCaseNamingPolicy = New JsonLowerCaseNamingPolicy
	...
	Private Function ToJson(Of T)(value As T) As String
		...
		Dim opts = New JsonSerializerOptions With {
					.PropertyNamingPolicy = If(IgnoreCase, JsonLowerCaseNamingPolicy, Nothing),
					.DictionaryKeyPolicy = If(IgnoreCase, JsonLowerCaseNamingPolicy, Nothing),
					.WriteIndented = False
				}
...
#Region "GET"
	...
	Public Function GetAll(Of T)(Optional ByRef errorMessage As String = "") As T
		Return ExecuteResult(Function() Client.JsonGet(Name).FromJson(Of T), errorMessage)
	End Function
	...
	Public Function [Get](path As String, Optional ByRef errorMessage As String = "") As String()
		...
		Return ExecuteResult(Function()
								 Dim str = Client.JsonGet(Name, Nothing, Nothing, Nothing, path)
								 If str.IsEmpty Then Return Nothing
								 If Not path.StartsWith("$.") AndAlso Not path.StartsWith("'$.") Then str = $"[{str}]"
	...
	Public Function [Get](Of T)(path As String, Optional ByRef errorMessage As String = "") As T()
		...
		Return ExecuteResult(Function()
								 ...
								 Return str.FromJson(Of T())
							 End Function, errorMessage)
	End Function

	''' <summary>获取数据</summary>
	''' <param name="paths">多个路径</param>
	Public Function [Get](paths As String(), Optional ByRef errorMessage As String = "") As IDictionary(Of String, String())
		paths = GetPath(paths, errorMessage)
		If paths.IsEmpty Then Return Nothing

		If paths.Length = 1 Then
			Dim value = [Get](paths(0), errorMessage)
			If value Is Nothing Then Return Nothing

			Return New Dictionary(Of String, String()) From {{paths(0), value}}
		End If

		' 检查路径中是否存在 $ 开头的项目，存在则结果会以数组返回
		Dim has = paths.Any(Function(x) x.StartsWith("$"c) OrElse x.StartsWith("'$"))

		Return ExecuteResult(Function()
								 Dim str = Client.JsonGet(Name, Nothing, Nothing, Nothing, paths)
								 If str.IsEmpty Then Return Nothing

								 Dim data = str.ToJsonDictionary
								 If data.IsEmpty Then Return Nothing

								 Return data.ToDictionary(Function(x) x.Key, Function(x)
																				 Dim s = JsonExtension.ToJson(x.Value)
																				 If Not has Then s = $"[{s}]"

																				 Return s.ToJsonList.Select(Function(y) JsonExtension.ToJson(y, True, False, False)).ToArray
																			 End Function)
							 End Function, errorMessage)
	End Function
	...
	''' <summary>获取数据</summary>
	''' <param name="paths">多个路径</param>
	Public Function [Get](Of T)(paths As String(), Optional ByRef errorMessage As String = "") As IDictionary(Of String, T())
		paths = GetPath(paths, errorMessage)
		If paths.IsEmpty Then Return Nothing

		If paths.Length = 1 Then
			Dim value = [Get](Of T)(paths(0), errorMessage)
			If value Is Nothing Then Return Nothing

			Return New Dictionary(Of String, T()) From {{paths(0), value}}
		End If
...
#Region "SET"
	...
	Public Function [Set](data As IDictionary(Of String, Object), Optional ByRef errorMessage As String = "") As Boolean
		...
		errorMessage = ""
		Dim paths = If(IgnoreCase, data.Keys.Select(Function(x) x.ToLowerInvariant).ToArray, data.Keys.ToArray)
...
#Region "ARRAY"
	...
	Public Function ArrayAppend(path As String, values As String(), Optional ByRef errorMessage As String = "") As Long()
		path = GetPath(path, errorMessage)
		If path.IsEmpty Then Return Nothing

		Return ExecuteResult(Function() Client.JsonArrAppend(Name, path, values), errorMessage)
	End Function
	...
	''' <param name="values">多个值</param>
	''' <remarks>返回数组的长度</remarks>
	Public Function ArrayAppend(Of T)(path As String, values As IEnumerable(Of T), Optional ByRef errorMessage As String = "") As Long()
		Dim data = values.Select(Function(x) ToJson(x)).ToArray
		Return ArrayAppend(path, data, errorMessage)
	End Function
	...
	''' <remarks>返回数组的长度</remarks>
	Public Function ArrayInsert(path As String, values As String(), Optional index As Long = 0, Optional ByRef errorMessage As String = "") As Long()
		path = GetPath(path, errorMessage)
		If path.IsEmpty Then Return Nothing

		Return ExecuteResult(Function() Client.JsonArrInsert(Name, path, index, values), errorMessage)
	End Function
	...
	''' <param name="index">开始位置，不设置则从最前面插入</param>
	''' <remarks>返回数组的长度</remarks>
	Public Function ArrayInsert(Of T)(path As String, values As IEnumerable(Of T), Optional index As Long = 0, Optional ByRef errorMessage As String = "") As Long()
		Dim data = values.Select(Function(x) ToJson(x)).ToArray
		Return ArrayInsert(path, data, index, errorMessage)
	End Function
